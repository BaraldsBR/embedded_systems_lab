...
int main(int argc, char *argv[])
{
    ...
    else printf("sudo ./spi-pigpio-speed [bps]\n\n");
    ...
    printf("choose numbers a and b: ");
    scanf("%hhd %hhd", &operator1, &operator2);
    
    buf[0] = (char) operator1;
    buf[1] = (char) operator2;
    buf[2] = '3'; 
    /* 
       this position is accessed when calling for 3-byte long
       transfer with the spi, so it is probably a bad idea
       to have it be uninitialized, '3' is just 00110011 in
       binary, it changes nothing in the code
    */
    spiXfer(spiDevice, speed, buf, buf, 3);

    printf("a + b = %hhd", (int8_t) buf[2]);
